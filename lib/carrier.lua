  ...
  for _, item in pairs(get_logistics_contents(network)) do
    ...
    if consumer then
      ...
      if missing > 0 then
        ...
        local source_inventory = get_logistics_or_deconstruction_source_inventory(source, item_id)
...
function update_carrier(record)
  if record.carrier_state == "move-to-source" then
    ...
    local inventory = get_logistics_or_deconstruction_source_inventory(source, record.carrier_item)
